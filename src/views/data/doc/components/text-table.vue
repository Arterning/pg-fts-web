<template>
  <a-layout class="flex-layout">
    <a-card :title="$t('Text')" class="general-card">
      <a-row>
        <a-col :flex="62">
          <a-form
...
<script lang="ts" setup>
  ...
  const submitNewOrEdit = async () => {
    ...
    try {
      if (buttonStatus.value === 'new') {
        ...
        await fetchApiList({ page: 1, size: pagination.pageSize, type: 'text' });
      } else {
  ...
  fetchApiList({ page: 1, size: pagination.pageSize, type: 'text' });
  ...
  const onPageSizeChange = async (pageSize: number) => {
    ...
    await fetchApiList({ page: 1, size: pageSize, type: 'text' });
  };
